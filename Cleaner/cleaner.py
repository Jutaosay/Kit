...
def scan_cleaner(
    ...
    installed_names = {normalize_name(a.name) for a in installed_apps if a.name}
    findings: list[Finding] = []
    ...
    for root in roots:
        ...
        for d in iter_child_dirs(root, max_depth=2):
            ...
            if name_norm and name_norm not in installed_names and name_norm not in portable_names:
                ...
                if in_appdata_or_pf and size_mb >= 30:
                    findings.append(
                        Finding(
                            ...
                            path=str(d),
                            size_mb=round(size_mb, 1),
                            reason="Directory name not matched in installed/portable app list",
                            game_like=game_like,
                        )
                    )
    ...
    final_findings = sorted(uniq.values(), key=lambda x: x.size_mb, reverse=True)
...
def print_result(res: ScanResult, top_n: int = 50) -> None:
    ...
    for f in res.findings[:top_n]:
        table.add_row(f.category, f"{f.size_mb:.1f}", "yes" if f.game_like else "no", f.path, f.reason)
